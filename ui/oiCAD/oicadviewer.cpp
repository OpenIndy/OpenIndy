...
OiCadViewer::OiCadViewer(QWidget* parent):QWidget(parent)
{
    ...
    aViewer->SetDefaultBackgroundColor(Quantity_NOC_WHITE);
    ...
    myView->SetSize(100.0);
    myView->SetZSize(100.0);
...
*/
QPaintEngine* OiCadViewer::paintEngine() const
{
  return 0;
}

void OiCadViewer::resizeEvent(QResizeEvent * e)
{
    if (!myView.IsNull())
    {
        myView->MustBeResized();
...
void OiCadViewer::mousePressEvent(QMouseEvent * mpEvent)
{
...
void OiCadViewer::mouseReleaseEvent(QMouseEvent * mrEvent)
{
...
void OiCadViewer::mouseDoubleClickEvent(QMouseEvent * mdcEvent)
{
    emit mouseDoubleClick(mdcEvent->modifiers(), mdcEvent->x(), mdcEvent->y());
}


void OiCadViewer::setDegenerateMode(const bool on)
{
