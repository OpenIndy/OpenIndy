...
win32 {
    ...
    win32:CONFIG(release, debug|release) {
        ...
        exists($$PWD/../bin/release/oisystemdb.sqlite) {
            message("system database is already present")
        }else{
            database.path = $$PWD/../bin/release
            database.files += $$PWD/../db/oisystemdb.sqlite
            INSTALLS += database
        }

    }else:win32:CONFIG(debug, debug|release){

        exists($$PWD/../bin/debug/oisystemdb.sqlite) {
            message("system database is already present")
        }else{
            database.path = $$PWD/../bin/debug
            database.files += $$PWD/../db/oisystemdb.sqlite
            INSTALLS += database
        }
